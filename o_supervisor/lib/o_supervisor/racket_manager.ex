defmodule OSupervisor.RacketManager do
  @moduledoc """
  Manages a Racket process instance via Erlang Port.

  Responsibilities:
  - Start and monitor Racket process
  ...
  def init(opts) do
    ...
    Logger.info("Starting RacketManager for role: #{role}")

    # Start Racket process
    port = start_racket_process(role, opts)
  ...
  def handle_call({:send_message, message}, _from, state) do
    case send_to_racket(state.port, message) do
      :ok -> {:reply, :ok, state}
  ...
  def handle_info({port, {:data, data}}, %{port: port} = state) do
    case Msgpax.unpack(data) do
      {:ok, message} ->
        handle_racket_message(message, state)
  ...
  def handle_info({port, {:exit_status, status}}, %{port: port} = state) do
    Logger.error("Racket process exited with status: #{status}")
    {:stop, {:racket_exit, status}, state}
  end
  ...
  def handle_info(:heartbeat_check, state) do
    ...
    if now - state.last_heartbeat > timeout do
      Logger.error("Racket process #{state.role} heartbeat timeout, restarting...")
  ...
  defp start_racket_process(role, opts) do
    config = Application.get_all_env(:o_supervisor)
    executable = config[:racket_executable] || "racket"
    main_script = config[:racket_main_script] || "../racket/o/main.rkt"
    ...
    Port.open(
      ...
      [
        ...
        {:env,
         [
           {'RACKET_HOME', String.to_charlist(System.get_env("RACKET_HOME") || "/usr/local/racket")},
           {'ELIXIR_MODE', 'true'}
         ]}
      ]
    )
  ...
  defp send_to_racket(port, message) do
    try do
      ...
      e ->
        Logger.error("Failed to send message to Racket: #{inspect(e)}")
        {:error, :send_failed}
    end
  end

  defp handle_racket_message(%{"type" => "heartbeat"}, state) do
    {:noreply, %{state | last_heartbeat: System.monotonic_time(:millisecond)}}
  end

  defp handle_racket_message(%{"type" => "checkpoint", "data" => snapshot}, state) do
    ...
    send_to_racket(state.port, ack)

    {:noreply, %{state | checkpoint_id: checkpoint_id, wal_buffer: :queue.new()}}
  end

  defp handle_racket_message(%{"type" => "wal_entry", "data" => entry}, state) do
    # Add to buffer
    ...
    new_buffer =
      if :queue.len(new_buffer) > 100 do
  ...
  defp handle_racket_message(%{"type" => "wal_batch", "data" => entries}, state) do
    Enum.each(entries, &OSupervisor.WALManager.append_entry/1)
    {:noreply, state}
  end

  defp handle_racket_message(
         %{"type" => "evolution_intent", "hypothesis" => hypothesis},
         state
       ) do
  ...
  defp handle_racket_message(%{"type" => "metrics", "data" => metrics}, state) do
    # Update health monitor
    OSupervisor.HealthMonitor.record_metrics(state.role, metrics)
    {:noreply, %{state | metrics: metrics}}
  end

  defp handle_racket_message(message, state) do
