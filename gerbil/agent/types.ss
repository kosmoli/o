...
(defstruct agent-config
  (id                    ; Agent ID (string)
   ...
   llm-config            ; LLM configuration (make-hash-table)
...
(defstruct agent-state
  (agent-id              ; Agent ID (string)
   ...
   metadata)             ; Additional state metadata (make-hash-table)
  transparent: #t)
...
(defstruct execution-step
  (id                    ; Step ID (string)
   ...
   metadata)             ; Additional step metadata (make-hash-table)
  transparent: #t)
...
(defstruct execution-context
  (agent-id              ; Agent ID (string)
   ...
   metadata)             ; Additional context metadata (make-hash-table)
  transparent: #t)
...
(defstruct execution-result
  (success               ; Was execution successful? (boolean)
   ...
   metadata)             ; Additional result metadata (make-hash-table)
  transparent: #t)
...
(def (make-default-agent-config agent-id name)
  ...
  (make-agent-config
   ...
   (let ((ht (make-hash-table)))
     (hash-put! ht 'temperature 0.7)
     (hash-put! ht 'max_tokens 4096)
     ht)
   50
   100000
...
(def (agent-config->hash config)
  ...
  (let ((ht (make-hash-table)))
  (hash-put! ht 'id (agent-config-id config))
  ht))
